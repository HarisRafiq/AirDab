...
@implementation MediaTableView_Ipad
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    {MPMediaItem *temp = [[[AudioPlayer sharedInstance] artistsArray] objectAtIndex:indexPath.row];
        ...
        UIImage *finalImage=[artWork imageWithSize:CGSizeMake(70, 70)] ;
...
{
    CircleCell_IPad *cell = (CircleCell_IPad*)[tView dequeueReusableCellWithIdentifier:CellIdentifier];
    if( !cell )
    {
        cell = [[CircleCell_IPad alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:CellIdentifier];
    ...
    if(indexPath.row>=[[AudioPlayer sharedInstance] artistsArray].count )
    {
        ...
        UIImage *finalImage=[artWork imageWithSize:CGSizeMake(78, 78)] ;
        if(finalImage==nil){
...
-(void)setupLeftAndRightLabels{
    ...
    _leftLabel.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:30];
    _leftLabel.textAlignment=NSTextAlignmentCenter;
    _rightLabel.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:30];
