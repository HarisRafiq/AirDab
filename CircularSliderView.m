...
static float barRadius, knobRadius = 2, knobAngle, minValue, maxValue;
...
-(void)setValue:(float)s{
    ...
    float percentDone = 1-s;
    knobAngle = MIN_ANGLE+(percentDone*(MAX_ANGLE-MIN_ANGLE));
    [self setNeedsDisplay];

}
-(void)setMinValue:(float)minimumValue maxValue:(float)maximumValue initialValue:(float)initialValue
{
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSetLineWidth(context, 4.0);
   CGContextSetStrokeColorWithColor(context, [UIColor whiteColor].CGColor);
    CGContextAddArc(context, knobCenter.x, knobCenter.y, knobRadius, 0, 2*M_PI, 0);
    /*
